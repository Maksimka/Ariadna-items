 ...
 */
public class PacketTableMouseListener extends MouseAdapter {

    private static final Logger logger = LoggerFactory.getLogger(PacketTableMouseListener.class);
    ...
    public void mouseClicked(MouseEvent event) {
        if (event.getButton() == MouseEvent.BUTTON1 && event.getClickCount() == 2) {
            ...
            PacketTable deliveryPacketTable = (PacketTable) event.getSource();
            int selectedRow = deliveryPacketTable.getSelectedRow();
            if (selectedRow >= 0) {
                PacketTableModel model = (PacketTableModel) deliveryPacketTable.getModel();
                DeliveryPacket deliveryPacket = model.getDeliveryPacket(selectedRow);
                logger.debug(deliveryPacket.toString());
                CreatePacketDialog packetDialog = new CreatePacketDialog(deliveryPacket);
